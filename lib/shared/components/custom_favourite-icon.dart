...
class CustomFavouriteIcon extends StatelessWidget {
  ...
  CustomFavouriteIcon({this.iconSize = 25.0, required this.productId});

  Widget build(BuildContext context) {
    return BlocConsumer<HomeCubit, HomeStates>(
        listener: (context, state) {},
        builder: (context, state) {
          return (HomeCubit.get(context).favourites.isNotEmpty)
              ? LikeButton(
                  circleColor:
                      CircleColor(start: Colors.redAccent, end: Colors.orange),
                  ...
                  likeBuilder: (_) => Icon(
                    Icons.favorite,
                    size: iconSize,
                    color: HomeCubit.get(context).favourites[productId]
                        ? red
                        : grey,
                  ),
                )
              : CustomLoading(size: 20.0);
        });
  }
